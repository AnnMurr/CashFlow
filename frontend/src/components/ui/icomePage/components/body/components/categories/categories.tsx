    ...
    useEffect(() => {
        const deleteCategory = async (event: MouseEvent) => {
            ...
            if (target.classList.contains("item-btn")) {
                ...
                const categoriesExpenses: Array<CategoriesExpensesType> = userDataFromStorage.data.categoriesIncome;
                const updatedUserData = categoriesExpenses.filter((item) => item.name !== categoryName);
    
                try {
                    userDataFromStorage.data.categoriesIncome = [...updatedUserData];
